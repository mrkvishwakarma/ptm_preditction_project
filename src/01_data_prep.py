...
def load_and_prepare_data(raw_train_path: str, raw_test_path: str, output_dir: str):
    """
    Loads raw data, inspects it, creates shuffled windows, and saves the prepared data.
    """
    print(f"Loading raw train data from {raw_train_path}...")
    train_df = pd.read_csv(raw_train_path)
    ...
    # --- NEW: Initial Data Inspection ---
    print("\n" + "=" * 50)
    print("🔍 INITIAL DATA INSPECTION (TRAIN SET)")
    print("=" * 50)
    print(f"Total Rows:    {train_df.shape[0]:,}")
    print(f"Total Columns: {train_df.shape[1]}")
    print(f"\nColumns Present: {list(train_df.columns)}")
    ...
    # Using dropna=False to also show if any Labels are completely missing
    print(train_df['Label'].value_counts(dropna=False))

    print("\n--- Data Preview (First 3 Rows) ---")
    # Temporarily widen pandas output to ensure the preview doesn't aggressively wrap
    with pd.option_context('display.max_columns', None, 'display.width', 1000):
        print(train_df.head(3))

    print("\n" + "=" * 50)
    print("🔍 INITIAL DATA INSPECTION (TEST SET)")
    print("=" * 50)
    ...
    # --- Step 3: Create Randomized Window ---
    print("--- Step 3: Creating Randomized Windows ---")
    print("Applying shuffling to training data...")
    train_df['shuffled_Seq'] = train_df['Seq'].apply(create_randomized_window)
    train_df['shuffled_full_sequence'] = train_df.apply(reconstruct_full_sequence, axis=1)
